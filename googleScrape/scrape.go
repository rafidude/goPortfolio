...
func GoogleScrape(searchTerm, countryCode, languageCode string, pages, count, backoff int) ([]SearchResult, error) {
	results := []SearchResult{}
	resultCounter := 0
...
func main() {
	res, err := GoogleScrape("rafi dudekula", "com", "en", 1, 30, 10)
	if err == nil {
		for _, res := range res {
			fmt.Println(res)
		}
